...
def intrinsic_reward(state, action, next_state, info=None, revised_state=None):
    ...
    reward += 0.026 * max(0.0, backbone_next - backbone_now)
    reward += 0.020 * max(0.0, road_comm_next - road_comm_now)
    reward += 0.008 * max(0.0, float(np.mean(next_state[9:12]) - np.mean(state[9:12])))
